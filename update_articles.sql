-- =====================================================
-- Mise à jour des articles avec contenu complet
-- Portfolio de Roukayath Gazaliou
-- =====================================================

-- Article 1 : Comment j'ai appris Laravel en autonomie pendant mon stage
UPDATE `articles` SET `contenu` = '<h2>Introduction</h2>
<p>Quand j\'ai débuté mon stage chez SolDigit en 2025, Laravel n\'était pas encore dans ma boîte à outils. Pourtant, l\'équipe travaillait exclusivement avec ce framework depuis plusieurs années. J\'ai dû apprendre vite pour être productive dès les premières semaines.</p>
...
<p>Les routes Laravel définissent comment mapper les URL aux contrôleurs. Le middleware filtre les requêtes avant qu\'elles n\'atteignent l\'application (authentification, CORS, etc.).</p>
<blockquote>
...
<li>Je crais un fichier de notes avec les pièges à éviter</li>
...
<li>⚙️ Pratique immédiate sur de vrais projets</li>
...
WHERE `slug` = 'apprendre-laravel-autonomie-stage';

-- Article 2 : Les bases du design UI/UX avec Figma pour développeurs
UPDATE `articles` SET `contenu` = '<h2>Pourquoi un développeur doit apprendre Figma</h2>
...
<li>🏗️ <strong>Construire des produits meilleurs</strong> : Vous pensez UX dès la conception, pas après</li>
...
WHERE `slug` = 'design-ui-ux-figma-developpeurs';

-- Article 3 : PHP OOP : les concepts essentiels à maîtriser en 2025
UPDATE `articles` SET `contenu` = '<h2>Pourquoi la POO en PHP est essentielle</h2>
...
<p>Cet article vous présente les pilliers de la POO en PHP avec des exemples concrets.</p>
...
echo $dev->afficher(); // Bob (bob@dev.com) - Spécialité : Laravel</code></pre>
...
<p>Après avoir exécuté <code>composer install</code>, vos classes se chargent automatiquement.</p>
...
WHERE `slug` = 'php-oop-concepts-essentiels-2025';
